Login_Silver()
{
	...
	lr_start_transaction("S02_UserLogin_Silver");
/*
	...
	web_custom_request("AWc7nCRAeesaa3d7C6NSzroEywK1AAAAAAAAAABJf4-AlTvwnJLWw4DjWa6Yt3mEoPGVRFwUMzLBxL9kcmWY8RUpSAxyzjR7ZDZpsyYp17c1HmkWGn3YHjVgIV-G8hefWCNg1OZDCx8Q8iWQ31d8_z5wVxjUA2gq0pAea7ANWIWyxeTj9wl9vl1Pe1QevK0xF_rl7JkOKBtVqgLdBGxvlNA4LCgiVC2Qi_OF1TgvFw5ehqeSjufwJL-JjWb2zMvp_4", 
		...
		"passwordExpiring\":false,\"usernameRecovery\":{\"type\":\"HPID Username Recovery Flow Handler\",\"$ref\":\"https://directory.stg.cd.id.hp.com/directory/v1/authentication/account/HPID%20Username%20Recovery%20Flow%20Handler/"
		"{cp_UserNameFlowHandler}\"},\"passwordRecovery\":{\"type\":\"HPID Password Recovery Flow Handler\",\"$ref\":\"https://directory.stg.cd.id.hp.com/directory/v1/authentication/account/HPID%20Password%20Recovery%20Flow%20Handler/"
		"{cp_PasswordFlowHandler}\"},\"status\":\"ready\",\"username\":\"{p_silverUser}@hpid\",\"password\":\"{p_password}\"},\"urn:hpid:governance:authenticator:customCaptcha:1.0\":{\"captchaKey\":\"{cp_CaptchaKey}=:1599034250830\",\"captchaImage\":\"{cp_CaptchaImage}\",\"required\":true,\"recaptchaKey\":\"{cp_ReCaptchaKey}\",\"status\":\"ready\"},\"urn"
		":hpid:governance:authenticator:checkPasswordAge:1.0\":{\"userName\":\"{p_silverUser}\",\"clientName\":\"{cp_ClientName}\"}}", 
		LAST);
...
*/
	...
	web_custom_request("username-password_2", 
		...
		"Mode=HTML", 
		"EncType=application/json;charset=UTF-8", 
		"Body={\"username\":\"{p_silverUser}@hpid\",\"password\":\"{p_password}\"}", 
		LAST);
	
	web_add_header("Upgrade-Insecure-Requests", 
		"1");
	
	web_reg_save_param("cp_KryptoNew","LB=krypto=","RB=&ddkey","ORD=1",LAST);
	...
	if(atoi(lr_eval_string("{Register}"))>0)
        {
			lr_end_transaction("S02_UserLogin_Silver",LR_PASS);
        }
    else
        {
    		lr_end_transaction("S02_UserLogin_Silver",LR_FAIL);
	...
	lr_save_string(lr_eval_string("{p_silverUser}"), "p_userEmail");
	
	lr_save_string("?pStoreID=bizclubsilver","StoreType");
	lr_save_string("&pStoreID=bizclubsilver","StoreType1");
