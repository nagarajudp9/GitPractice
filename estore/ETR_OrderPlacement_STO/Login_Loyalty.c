Login_Loyalty()
{
	...
	lr_start_transaction("S02_UserLogin_Loyalty");
	
//	lr_start_sub_transaction("S02-1_HPID_UserLogin_Loyalty","S02_UserLogin_Loyalty");
/*
	...
	web_custom_request("AWc7nCRAeesaa3d7C6NSzroEywK1AAAAAAAAAABJf4-AlTvwnJLWw4DjWa6Yt3mEoPGVRFwUMzLBxL9kcmWY8RUpSAxyzjR7ZDZpsyYp17c1HmkWGn3YHjVgIV-G8hefWCNg1OZDCx8Q8iWQ31d8_z5wVxjUA2gq0pAea7ANWIWyxeTj9wl9vl1Pe1QevK0xF_rl7JkOKBtVqgLdBGxvlNA4LCgiVC2Qi_OF1TgvFw5ehqeSjufwJL-JjWb2zMvp_4", 
		...
		"passwordExpiring\":false,\"usernameRecovery\":{\"type\":\"HPID Username Recovery Flow Handler\",\"$ref\":\"https://directory.stg.cd.id.hp.com/directory/v1/authentication/account/HPID%20Username%20Recovery%20Flow%20Handler/"
		"{cp_UserNameFlowHandler}\"},\"passwordRecovery\":{\"type\":\"HPID Password Recovery Flow Handler\",\"$ref\":\"https://directory.stg.cd.id.hp.com/directory/v1/authentication/account/HPID%20Password%20Recovery%20Flow%20Handler/"
		"{cp_PasswordFlowHandler}\"},\"status\":\"ready\",\"username\":\"{p_loyaltyUser}@hpid\",\"password\":\"{p_password}\"},\"urn:hpid:governance:authenticator:customCaptcha:1.0\":{\"captchaKey\":\"{cp_CaptchaKey}=:1599034250830\",\"captchaImage\":\"{cp_CaptchaImage}\",\"required\":true,\"recaptchaKey\":\"{cp_ReCaptchaKey}\",\"status\":\"ready\"},\"urn"
		":hpid:governance:authenticator:checkPasswordAge:1.0\":{\"userName\":\"{p_loyaltyUser}\",\"clientName\":\"{cp_ClientName}\"}}", 
		LAST);
...
*/
	...
	web_custom_request("username-password_2", 
		...
		"Mode=HTML", 
		"EncType=application/json;charset=UTF-8", 
		"Body={\"username\":\"{p_loyaltyUser}@hpid\",\"password\":\"{p_password}\"}", 
		LAST);
	
//	lr_end_sub_transaction("S02-1_HPID_UserLogin_Loyalty",LR_AUTO);

//	lr_start_sub_transaction("S02-2_ETR_UserLogin_Loyalty","S02_UserLogin_Loyalty");	
/*
...
*/	
	...
	if(atoi(lr_eval_string("{Register}"))>0)
        {
		  lr_end_transaction("S02_UserLogin_Loyalty",LR_PASS);
  //      lr_end_sub_transaction("S02-2_ETR_UserLogin_Loyalty",LR_PASS);
        }
    else
        {
    		lr_end_transaction("S02_UserLogin_Loyalty",LR_FAIL);
  //          lr_end_sub_transaction("S02-2_ETR_UserLogin_Loyalty",LR_FAIL);
            lr_exit(LR_EXIT_MAIN_ITERATION_AND_CONTINUE,LR_FAIL);
        }	
	

	//lr_end_transaction("S02_UserLogin_Loyalty",LR_AUTO);
	
	LoginFlag=1;
	
	loyaltyFlag=1;
	
	lr_save_string(lr_eval_string("{p_loyaltyUser}"), "p_userEmail"); 
	lr_save_string("","StoreType");
	
	
	if(atoi(lr_eval_string("{p_randomNo}"))<5)
	{
	
	//web_reg_find("Text=Points expire 180 days from date earned",LAST);
	
	lr_start_transaction("S04_MyRewards");
	...
	web_url("HPServices", 
		"URL=https://{p_hostName}/us-en/shop/HPServices?langId=-1&storeId=10151&catalogId=10051&pstoreId=&_=1599298745430&action=cupids&catentryId=&modelId=", 
		"Resource=0", 
		"RecContentType=text/html", 
	...
	lr_end_transaction("S04_MyRewards", LR_AUTO);
