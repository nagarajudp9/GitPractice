MDP()
{
	...
	web_url("p_MDP", 
		"URL=https://{p_hostName}/us-en/shop/mdp/{p_MDP}{StoreType}", 
		"Resource=0", 
	...
	MDPcount=atoi(lr_eval_string("{cp_MDPcatentryID_count}"));	
		...
		lr_end_transaction("S07_MDP",LR_FAIL);
...

	{
  	...
  	//lr_start_sub_transaction("S17-1_DealsAPI",(lr_eval_string("{AgentType}S07_MDP")));
	web_save_timestamp_param("cp_time", LAST);
	web_url("dealsapi", 
	...
	web_url("HPServices_2", 
	  //"URL=https://{p_hostName}/us-en/shop/HPServices?_={cp_time}&action=ipd&catentryId={MDPURL}&modelId=&langId=-1&storeId=10151&catalogId=10051", 
		"URL=https://{p_hostName}/us-en/shop/HPServices?_={cp_time}&action=cupids&catentryId={MDPURL}&modelId=&langId=-1&storeId=10151&catalogId=10051",		
		"Resource=0", 
		"RecContentType=text/html", 
		"Referer=https://{p_hostName}/us-en/shop/mdp/{p_MDP}", 
		"Snapshot=t22.inf", 
		...
		{
			lr_end_transaction(lr_eval_string("{AgentType}S07_MDP"),LR_PASS);
			AddToCartFlag = 1;
