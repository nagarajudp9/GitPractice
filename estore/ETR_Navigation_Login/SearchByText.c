...
web_reg_save_param("cp_facet01","LB=,\"Field\":\"","RB=\",\"","ord=ALL",LAST);
	...
	else if(pmaxValue>0 && pmaxValue<3)
	{
	
	lr_save_string(lr_paramarr_random("cp_facet"),"cp_facet");
	lr_save_string(lr_paramarr_random("cp_facet01"),"cp_facet01");
	
	web_reg_save_param("cp_facetValue01","LB={cp_facet}\":[\"","RB=\"]","ord=ALL","Notfound=Warning",LAST);
	web_reg_save_param("cp_facetValue02","LB={cp_facet01}\":[\"","RB=\"]","ord=ALL","Notfound=Warning",LAST);
...
web_reg_save_param("sbtcatentryid","lb=\"catentry_id\":[\"","rb=\"],","ord=all",LAST);
	...
	web_custom_request("search_8", 
		...
		"Body={\"query\":\"type:product\",\"ClientData\":{\"VisitId\":\"2fa8087b-50af-4655-84ac-6cb481124f9e\",\"VisitorId\":\"167d305c-b8c0-42b7-8fd3-196fcfe5a2d5\",\"UserAgent\":\"Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/105.0.0.0 Safari/537.36\",\"Custom\":{}},\"FacetSelections\":{\"{cp_facet}\":[\"{cp_facetValue01}\"],\"{cp_facet}\":[\"{cp_facetValue02}\"]},\"SortBy\":\"{cp_SortValue}\",\"Keyword\":\"{p_Search}\",\"ClientGuid\":\"cc79148f6e9a47cbbbe8b0e3ae0f6e89\"}", 
		LAST);
	...
	web_url("HPServices_7", 
		...
		"RecContentType=text/html", 
		//"Referer=https://{p_hostName}/us-en/shop/sitesearch?{cp_facet}={cp_facetValue01}&keyword={p_Search}&sort={cp_SortValue}",
		"Referer=https:///us-en/shop/sitesearch?dte_facet_{cp_facet}_type={cp_facetValue01}&{cp_facet}_status={cp_facetValue02}&keyword={cp_SortValue}",		
		"Snapshot=t42.inf", 
		"Mode=HTML", 
		LAST);
