Login_HPEPP()
{
	...
	lr_start_transaction(lr_eval_string("{AgentType}S02_UserLogin_HPEPP"));
	...
	web_custom_request("username-password_2", 
		...
		"Mode=HTML", 
		"EncType=application/json;charset=UTF-8", 
		"Body={\"username\":\"{p_hpeppUser}@hpid\",\"password\":\"{p_password}\"}", 
		LAST);
	...
	if(atoi(lr_eval_string("{Register}"))>0)
        {
			lr_end_transaction(lr_eval_string("{AgentType}S02_UserLogin_HPEPP"),LR_PASS);
        }
    else
        {
    		lr_end_transaction(lr_eval_string("{AgentType}S02_UserLogin_HPEPP"),LR_FAIL);
	...
	lr_save_string(lr_eval_string("{p_hpeppUser}"), "p_userEmail"); 
	
	lr_save_string("?pStoreID=hpepp","StoreType");
lr_save_string("&pStoreID=hpepp","StoreType1");	
