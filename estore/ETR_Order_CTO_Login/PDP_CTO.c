...

{
	...
	web_reg_find("Text={p_PartNumber1}","savecount=C_PartNumberCount",LAST);
	web_save_timestamp_param("cp_time", LAST);
	
	web_convert_param("cp_ModelRefParam","SourceString={p_ModelRef1}","SourceEncoding=HTML","TargetEncoding=URL",LAST);
	
	web_reg_save_param("productName","LB=var productName = '","RB=';\r\n","Notfound=warning",LAST);
	web_reg_save_param("cp_CatEntryID","LB=10051&amp;catEntryId=","RB=&amp",LAST);
	
	//web_reg_save_param("cp_CatEntryID","LB=\"catentryId\":\"","RB=\"",LAST);
	//"catentryId":"3074457345618619818","
	web_reg_save_param("cp_PaySessionID","LB=JSESSIONID=0000","RB=:","Notfound=Warning",LAST);
	lr_start_transaction("S08_PDP_CTO");
	
	
	
	web_url("hp-laptop-15z-ef100-touch-optional-8ze47av-1", 
		"URL=https://{p_hostName}/us-en/shop/pdp/{cp_pdpURL1}", 
		"Resource=0", 
...
web_url("HPServices_2", 
	...
	web_url("async_2", 
		"URL=https://{p_hostName}/us-en/shop/app/api/web/graphql/page/pdp%2F{cp_pdpURL1}/async", 
		"Resource=0", 
...
web_reg_find("Text=\"lowStock\": true", "SaveCount=c_OutOfStock", LAST);
	...
	web_url("HPServices_3", 
		"URL=https://{p_hostName}/us-en/shop/HPServices?_=1627102073801&action=ipd&catentryId{cp_CatEntryID},{cp_CatEntryID}&modelId=&langId=-1&storeId=10151&catalogId=10051", 
		"Resource=0", 
		"RecContentType=text/html", 
		"Referer=https://{p_hostName}/us-en/shop/pdp/{cp_pdpURL1}", 
		"Snapshot=t10.inf", 
