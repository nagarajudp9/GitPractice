MyAccounts()
{
	...
	web_reg_find("Text=accountNameType","SaveCount=AsIdCount",LAST);
	...
	else if(prandNumber<6)
		
	{		
		web_set_sockets_option("CLOSE_KEEPALIVE_CONNECTIONS","1");		
	lr_start_transaction("CP37_AcctSearch_ByName");
	
	web_reg_find("Text=accountname","SaveCount=AsNameCount",LAST);
	web_reg_save_param("cp_AccountId","LB=\"account_id\": \"","RB=\",","ORD=ALL","NotFound=Warning",LAST);
	...
	web_submit_data("GetSMBPrivateAccountUsers_4", 
		...
		{
			lr_end_transaction("CP37_AcctSearch_ByName",LR_FAIL);
			CC_Logout();    		
        	lr_exit(LR_EXIT_MAIN_ITERATION_AND_CONTINUE,LR_FAIL);
		}
	...
	web_reg_find("Text=endUserEmail","SaveCount=AsEmailCount",LAST);
	web_reg_save_param("cp_AccountId","LB=\"account_id\": \"","RB=\",","ORD=ALL","NotFound=Warning",LAST);
	...
	web_set_sockets_option("CLOSE_KEEPALIVE_CONNECTIONS","1");		
		...
		web_reg_find("Text=actionName","SaveCount=ViewAccCount",LAST);
