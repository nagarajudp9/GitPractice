MyQuotes()
{
	...
	web_url("HPServices_3", 
		"URL=https://{p_hostName}/us-en/shop/HPServices?langId=-1&storeId=10151&catalogId=10051&_=1526298613304&action=cupis&catentryId=&modelId=&retainPOCart=false", 
		"TargetFrame=", 
		"Resource=0", 
		"RecContentType=text/html", 
	...
	if(atoi(lr_eval_string("{QsNoCount}")) > 0)
		...
		{
			//lr_end_transaction("CP43_QS_ByNo",LR_FAIL);
			lr_exit(LR_EXIT_MAIN_ITERATION_AND_CONTINUE,LR_FAIL);
	...
	if(atoi(lr_eval_string("{QsNoCount}")) > 0)
		...
		{
			lr_end_transaction("CP43_QS_ByDate",LR_FAIL);
			lr_exit(LR_EXIT_MAIN_ITERATION_AND_CONTINUE,LR_FAIL);
	...
	if(pmaxValue>0)
	{
		...
		web_url("HPServices_4", 
			"URL=https://{p_hostName}/us-en/shop/HPServices?langId=-1&storeId=10151&catalogId=10051&_=1526298736135&action=cupis&catentryId=&modelId=&retainPOCart=false", 
			"TargetFrame=", 
			"Resource=0", 
			"RecContentType=text/html", 
			"Snapshot=t38.inf", 
